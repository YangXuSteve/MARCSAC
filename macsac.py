...
class MACSAC(nn.Module):
    def __init__(self, args):
        super(MACSAC, self).__init__()
        self.args = args
        ...
        self.cqs = [ContinuousQFunction(sum(self.obs_dims), sum(self.action_dims), self.hid_size, self.hid_activation, self.cuda)
                    for i in range(self.agent_num)]
        self.lams = [nn.Parameter(torch.tensor(0.268, dtype=torch.float32, device=self.device)) for i in range(self.agent_num)]
        ...
        for i in range(self.agent_num):
            ...
            self.lam_parameters.append(self.lams[i])
    ...
    def _compute_loss_cq(self, data):
        ...
        with torch.no_grad():
            ...
            for i in range(self.agent_num):
                ...
                target_cq[i] = costs[i] + self.gamma * (1 - done) * cq_target[i]
    ...
    def _compute_loss_pi(self, i, data):
        ...
        loss_pi = (-q_eval + self.lams[i] * cq_eval).mean()
    ...
    def _compute_loss_lam(self, data):
        ...
        for i in range(self.agent_num):
            ...
            loss_lam += (-self.lams[i] * cq_eval[i]).mean()
            # print(f'Agent {i}: Lam: {self.lams[i]} Cq: {cq_eval[i]}')
    ...
    def update_lam(self, batch):
        ...
        for i in range(self.agent_num):
            if self.lams[i].detach().cpu().numpy() < 0.0:
                self.lams[i].data = torch.tensor(0.0, dtype=torch.float32, device=self.device)
            if self.lams[i].detach().cpu().numpy() > 20.0:
                self.lams[i].data = torch.tensor(20.0, dtype=torch.float32, device=self.device)
    ...
    def save_to_pth(self, path):
        state = {'pis': [self.pis[i].state_dict() for i in range(self.agent_num)],
                 ...
                 'lams': [self.lams[i] for i in range(self.agent_num)]}
    ...
    def load_from_pth(self, path):
        ...
        for i in range(self.agent_num):
            ...
            self.cq_targs[i].load_state_dict(saved_model['cq_targs'][i])
            self.lams[i] = saved_model['lams'][i]
