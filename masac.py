...
class MASAC(nn.Module):
    def __init__(self, args):
        ...
        self.pi_parameters = list()
        self.q_parameters = list()
        for i in range(self.agent_num):
            self.pi_parameters += list(self.pis[i].parameters())
        self.q_parameters += list(self.central_q.parameters())
        self.pi_optimizer = self.optimizer_class(self.pi_parameters, lr=args.policy_lrate)
        ...
        self.pi_scheduler = torch.optim.lr_scheduler.ExponentialLR(self.pi_optimizer, 0.999)
    ...
    def _compute_loss_pi(self, data):
        ...
        entropy = -self.alpha * torch.cat(logp_acts, dim=-1).to(self.device).sum(dim=-1, keepdim=True)
        global_act = pack_actions_tensor(*acts).to(self.device)
    ...
    def update_q(self, batch):
        ...
        if self.pi_optimizer.param_groups[0]['lr'] >= self.policy_end_lrate:
    ...
    def load_from_pth(self, path):
        ...
        # self.pi_optimizer.load_state_dict(saved_model['pi_optimizer'])
        # self.q_optimizer.load_state_dict(saved_model['q_optimizer'])
